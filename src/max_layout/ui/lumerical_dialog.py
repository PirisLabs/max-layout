...
class LumericalExportDialog(QDialog):
    ...
    def _make_solver_tab(self) -> None:
        ...
        self.show_geometry_preview.setChecked(bool(self.saved.get("show_geometry_preview", True)))
        self.show_geometry_preview.setToolTip("Presentation-only diagnostic. Enabled by default; disabling it does not change the model.")
        self.show_port_mode_preview = QCheckBox("Render optional Ex/Ey port-mode images before solving")
        self.show_port_mode_preview.setChecked(bool(self.saved.get("show_port_mode_preview", True)))
        self.show_port_mode_preview.setToolTip("Enabled by default. Required polarization and effective-index selection still run when this diagnostic is disabled.")
